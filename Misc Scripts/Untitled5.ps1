...
#           manually after script completes.
    ...
    if ( !$publicIpId ) {
        ...
        $publicIp = Get-AzPublicIpAddress -ResourceGroupName $rgName3 -Name $publicIpName
    ...
    If ( !$Error ) {
        ...
        if ( $yn -eq "Yes" ) {
            ...
            # Reconfigure existing Network Interface for new Subnet
                ...
                Set-AzNetworkInterface -NetworkInterface $nicNew
...
#           script doesn't attempt to move them.
    ...
    if ( $nicRgName -ne $rgName ) {

        Move-AzResource `
            -ResourceId $nicId `
            -DestinationResourceGroupName $rgName
